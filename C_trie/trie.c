    ...
    while (fscanf(infile, "%s", name) == 1)
    {
        ...
        for (int i = 0, n = strlen(name); i < n; i++)
        {
            ...
            if (cursor->children[index] == NULL)
            {
                ...
                // link the new node to the first array
...
bool check(char *word)
{
    ...
    node *cursor = root; // creating a cursor to navigate through the trie
    // iterate through the word to search in the Trie Root
    for (int i = 0, n= strlen(word); i < n; i++)
    {
        // checking letter by letter
        index = tolower(word[i]) - 'a';
        // if word not complete and no children => return False bcause no corresponding letters registered in the tree
        if (cursor->children[index] == NULL)
        {
            return false;
        }
        // update cursor to the adress of the next node
        cursor = cursor->children[index];
    }
    // Check if the last node contains a word or if the path is just a subset of another longer name
